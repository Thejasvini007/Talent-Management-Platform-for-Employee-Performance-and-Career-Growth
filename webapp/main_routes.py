...
@login_required
def study_plan():
    weeks = db.user_learning_path(g.user["id"])
    return render_template("study_plan.html", weeks=weeks)


@bp.route("/study-plan/day/<int:day_id>/complete", methods=["POST"])
@login_required
def study_plan_mark_complete(day_id: int):
    db.mark_day_complete(day_id, g.user["id"])
    flash("Marked as complete — next day unlocked.", "ok")
