...
def create_learning_week(week_no: int, title: str, created_by: str) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO learning_paths (week_no, title, created_by, created_at) VALUES (?,?,?,?)",
            (week_no, title, created_by, _now()),
        )
        return cur.lastrowid


def add_learning_day(
    path_id: int, day_no: int, topic: str, document_id: int | None, auto_gen_exam: bool
) -> int:
    with get_conn() as conn:
        cur = conn.execute(
            "INSERT INTO learning_path_days (path_id, day_no, topic, document_id, auto_gen_exam)"
            " VALUES (?,?,?,?,?)",
            (path_id, day_no, topic, document_id, int(auto_gen_exam)),
        )
        return cur.lastrowid
...
def list_learning_paths() -> list[dict]:
    with get_conn() as conn:
        weeks = [dict(r) for r in conn.execute(
            "SELECT * FROM learning_paths ORDER BY week_no"
        ).fetchall()]
        for w in weeks:
            days = conn.execute(
                "SELECT ld.*, d.filename AS document_name"
                " FROM learning_path_days ld"
                " LEFT JOIN documents d ON d.id = ld.document_id"
                " WHERE ld.path_id = ? ORDER BY ld.day_no",
                (w["id"],),
            ).fetchall()
            w["days"] = [dict(d) for d in days]
        return weeks


def delete_learning_week(path_id: int) -> None:
    with get_conn() as conn:
        conn.execute("DELETE FROM learning_paths WHERE id = ?", (path_id,))


def set_day_exam(day_id: int, exam_id: int) -> None:
    with get_conn() as conn:
        conn.execute("UPDATE learning_path_days SET exam_id = ? WHERE id = ?", (exam_id, day_id))

def mark_day_complete(day_id: int, user_id: int) -> None:
    with get_conn() as conn:
        conn.execute(
            "CREATE TABLE IF NOT EXISTS learning_day_progress ("
            "id INTEGER PRIMARY KEY AUTOINCREMENT,"
        ...
        conn.execute(
            "INSERT OR IGNORE INTO learning_day_progress (day_id, user_id, completed_at)"
            " VALUES (?, ?, datetime('now'))",
            (day_id, user_id),
        )


def user_learning_path(user_id: int) -> list[dict]:
    """Weeks/days for a trainee. Reading days unlock via manual 'mark complete'.
    Exam days (day.exam_id set) unlock the next day once the exam is graded."""
    with get_conn() as conn:
        conn.execute(
            "CREATE TABLE IF NOT EXISTS learning_day_progress ("
            "id INTEGER PRIMARY KEY AUTOINCREMENT,"
            ...
            for d in days:
                ...
                if d["exam_id"]:
                    ...
                    if row and row["graded"]:
                        ...
                        status = "exam_not_assigned"
                        is_complete = False
                else:
                    prog = conn.execute(
                        "SELECT 1 FROM learning_day_progress WHERE day_id=? AND user_id=?",
                        (d["id"], user_id),
                    ).fetchone()
                    status = "read_complete" if prog else "unread"
                    is_complete = bool(prog)
                d["status"] = status
                d["unlocked"] = prev_complete
                day_list.append(d)
